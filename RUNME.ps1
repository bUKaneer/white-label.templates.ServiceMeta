...
Start-Process -Wait -NoNewWindow $DotNetExecutablePath -ArgumentList "sln", "add", "$UserInterfaceServerProjectFolder\UserInterface.csproj", "--solution-folder", "Services\$($ProjectName)UserInterface"

Start-Process -Wait -NoNewWindow $DotNetExecutablePath -ArgumentList "sln", "add", "$ApiProjectFolder\WebApi.csproj", "--solution-folder", "Services\$($ProjectName)WebApi"